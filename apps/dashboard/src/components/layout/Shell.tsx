...
import {
  ...
  ChevronLeft, ChevronRight, LogOut, Flame, Building2, ChevronDown,
} from 'lucide-react';
...
export default function Shell({ children }: { children: React.ReactNode }) {
  const { logout, appId, company, companyToken } = useAuth();
  const navigate = useNavigate();
  const [collapsed, setCollapsed] = useState(false);
  const [apps, setApps] = useState<Array<{ id: string; name: string }>>([]);
  const [activeAppId, setActiveAppId] = useState(() => localStorage.getItem('cl_active_app_id') ?? '');
  const [showAppPicker, setShowAppPicker] = useState(false);

  // Load apps list for the switcher
  useEffect(() => {
    if (!companyToken) return;
    companyApi.getApps(companyToken)
      .then((r) => setApps((r.data as { apps: Array<{ id: string; name: string }> }).apps))
      .catch(() => {});
  }, [companyToken]);
  ...
  const switchApp = (id: string) => {
    localStorage.setItem('cl_active_app_id', id);
    setActiveAppId(id);
                  ...
                  {apps.length > 0 && (
                    <div className="relative mt-1.5">
                      <button
                        ...
                        className="w-full flex items-center justify-between bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg px-2 py-1.5 text-xs text-slate-300 transition-colors"
                      >
                        <span className="truncate">{activeApp?.name ?? 'Select app'}</span>
                        <ChevronDown size={11} className="flex-shrink-0 ml-1 text-slate-500" />
                      </button>
                      {showAppPicker && (
                        <div className="absolute bottom-full left-0 right-0 mb-1 bg-slate-800 border border-white/10 rounded-lg overflow-hidden shadow-xl z-50">
                          {apps.map((a) => (
                            <button
                              key={a.id}
