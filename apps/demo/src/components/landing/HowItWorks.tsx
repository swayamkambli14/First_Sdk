...
const steps = [
  { num: '01', icon: Wallet, title: 'Connect Wallet',  desc: 'Sign in with MetaMask or WalletConnect — no email, no password.' },
  { num: '02', icon: Zap,    title: 'Take Actions',    desc: 'Every purchase, referral, or milestone fires a loyalty event.' },
  { num: '03', icon: Trophy, title: 'Earn Rewards',    desc: 'Points, badges, and spins land in your wallet automatically.' },
];
              ...
              <div key={step.num} className="flex-1 flex items-start gap-0">
                ...
                {i < steps.length - 1 && (
                  <div className="flex-1 flex items-start pt-8 min-w-[40px]">
                    <div className="w-full border-t-2 border-dashed border-cyan-500/25 mt-0" />
                  </div>
                )}
              </div>
            );
        ...
        <div className="hidden lg:flex items-start gap-0 relative">
          {steps.map((step, i) => (
            <div key={step.num} className="flex-1 flex items-start gap-0">
              <div className="flex flex-col items-center text-center px-8">
                <div className="relative mb-6">
                  <div className="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-2xl mb-1 group-hover:border-cyan-500/40">
                    {step.icon}
                  </div>
                  <span className="absolute -top-2 -right-2 font-['Space_Mono'] text-xs text-cyan-400 font-bold bg-[#0a0a0f] px-1">
                    {step.num}
                  </span>
                </div>
                ...
                <p className="font-['DM_Sans'] text-gray-400 text-sm leading-relaxed max-w-[200px]">
                  {step.desc}
                </p>
              </div>
            ...
            <div key={step.num} className="flex gap-5">
              <div className="flex flex-col items-center">
                <div className="relative">
                  ...
                  <span className="absolute -top-1.5 -right-1.5 font-['Space_Mono'] text-[10px] text-cyan-400 font-bold bg-[#0a0a0f] px-0.5">
                    {step.num}
                  </span>
                </div>
                {i < steps.length - 1 && (
              ...
              <div className="pb-8">
                <h3 className="font-['Space_Mono'] text-white font-bold text-sm mb-1">
                  {step.title}
                </h3>
