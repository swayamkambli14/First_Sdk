...
import { Auth, AuthLogoutResponse, AuthRequestNonceParams, AuthRequestNonceResponse, AuthVerifySignatureParams, Tier, UserProfile } from './resources/auth';
import { EventRetrieveStatusResponse, EventStatus, EventTrackParams, EventTrackResponse, EventType, Events } from './resources/events';
import { Leaderboard, LeaderboardPeriod, LeaderboardRetrieveParams, LeaderboardRetrieveResponse } from './resources/leaderboard';
import { ReferralEntry, ReferralRetrieveChainResponse, ReferralRetrieveStatsResponse, ReferralStatus, ReferralValidateCodeResponse, Referrals } from './resources/referrals';
import { Reward, RewardType, UserListBadgesResponse, UserListRewardsParams, UserListRewardsResponse, UserRetrievePointsResponse, Users } from './resources/users';
import { Admin } from './resources/admin/admin';
import { AppRegisterParams, AppRegisterResponse, AppRetrieveResponse, AppUpdateWebhookParams, AppUpdateWebhookResponse, Apps } from './resources/apps/apps';
...
const environments = {
  ...
  environment_1: 'https://api.chainloyalty.app/v1',
};
type Environment = keyof typeof environments;

export interface ClientOptions {
  /**
   * Raw API key issued by POST /apps/register (sk_live_… format). Used by SaaS server backends to submit events.
   *
   */
  apiKey?: string | undefined;

  /**
   * Static secret from env.ADMIN_SECRET. Guards all /admin/* routes.
   *
   */
   ...
   * - `environment_1` corresponds to `https://api.chainloyalty.app/v1`
   */
   ...
   *
   * Defaults to process.env['ASYNC_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['ASYNC_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Async API. 
 */
export class Async {
  apiKey: string;
  adminSecret: string;
   ...
   * API Client for interfacing with the Async API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['ASYNC_API_KEY'] ?? undefined]
   * @param {string | undefined} [opts.adminSecret=process.env['ASYNC_ADMIN_SECRET'] ?? undefined]
   * @param {Environment} [opts.environment=production] - Specifies the environment URL to use for the API.
   * @param {string} [opts.baseURL=process.env['ASYNC_BASE_URL'] ?? http://localhost:3000/v1] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('ASYNC_BASE_URL'),
    apiKey = readEnv('ASYNC_API_KEY'),
    adminSecret = readEnv('ASYNC_ADMIN_SECRET'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.AsyncError(
        'The ASYNC_API_KEY environment variable is missing or empty; either provide it, or instantiate the Async client with an apiKey option, like new Async({ apiKey: \'My API Key\' }).'
      );
    }
    if (adminSecret === undefined) {
      throw new Errors.AsyncError(
        'The ASYNC_ADMIN_SECRET environment variable is missing or empty; either provide it, or instantiate the Async client with an adminSecret option, like new Async({ adminSecret: \'My Admin Secret\' }).'
      );
    }
    ...
    if (baseURL && opts.environment) {
      throw new Errors.AsyncError(
        'Ambiguous URL; The `baseURL` option (or ASYNC_BASE_URL env var) and the `environment` option are given. If you want to use the environment you must pass baseURL: null'
      )
    }

    this.baseURL = options.baseURL || environments[options.environment || 'production'];
    this.timeout = options.timeout ?? Async.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('ASYNC_LOG'), 'process.env[\'ASYNC_LOG\']', this) ?? defaultLogLevel;
  ...
  protected async authHeaders(opts: FinalRequestOptions, schemes: { apiKeyAuth?: boolean, adminAuth?: boolean }): Promise<NullableHeaders | undefined> {
    return buildHeaders([schemes.apiKeyAuth ? await this.apiKeyAuth(opts) : null, schemes.adminAuth ? await this.adminAuth(opts) : null]);
  }
  ...
  protected async adminAuth(opts: FinalRequestOptions): Promise<NullableHeaders | undefined> {
    return buildHeaders([{ 'x-admin-secret': this.adminSecret }]);
  }
  ...
  private async buildHeaders({
    ...
    const headers = buildHeaders([
      ...
      ...getPlatformHeaders()},
      await this.authHeaders(options, options.__security ?? { apiKeyAuth: true,adminAuth: true }),
  ...
  static AsyncError = Errors.AsyncError;
   ...
   */
  leaderboard: API.Leaderboard = new API.Leaderboard(this);
  /**
...
Async.Events = Events;
Async.Users = Users;
Async.Referrals = Referrals;
Async.Leaderboard = Leaderboard;
Async.Apps = Apps;
Async.Admin = Admin;
