...
import { APIPromise } from 'async/core/api-promise';

import util from 'node:util';
import Async from 'async';
import { APIUserAbortError } from 'async';
...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['ASYNC_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: Async) => {
  ...
  test('default logLevel is warn', async () => {
    const client = new Async({ apiKey: 'My API Key', adminSecret: 'My Admin Secret' });
  ...
  test('warn when env var level is invalid', async () => {
    ...
    process.env['ASYNC_LOG'] = 'not a log level';
    const client = new Async({
  logger: logger,
  apiKey: 'My API Key',
  adminSecret: 'My Admin Secret',
    ...
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'ASYNC_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  adminSecret: 'My Admin Secret',
    ...
    afterEach(() => {
      process.env['ASYNC_BASE_URL'] = undefined;
    });
  ...
  adminSecret: 'My Admin Secret',
    ...
    test('env variable', () => {
      process.env['ASYNC_BASE_URL'] = 'https://example.com/from_env';
      const client = new Async({ apiKey: 'My API Key', adminSecret: 'My Admin Secret' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['ASYNC_BASE_URL'] = ''; // empty
      const client = new Async({ apiKey: 'My API Key', adminSecret: 'My Admin Secret' });
      expect(client.baseURL).toEqual('http://localhost:3000/v1')
    });

    test('blank env variable', () => {
      process.env['ASYNC_BASE_URL'] = '  '; // blank
      const client = new Async({ apiKey: 'My API Key', adminSecret: 'My Admin Secret' });
      expect(client.baseURL).toEqual('http://localhost:3000/v1')
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['ASYNC_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Async({ apiKey: 'My API Key', adminSecret: 'My Admin Secret' });
  ...
  adminSecret: 'My Admin Secret',
    ...
    // default
    const client2 = new Async({ apiKey: 'My API Key', adminSecret: 'My Admin Secret' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['ASYNC_API_KEY'] = 'My API Key';
    process.env['ASYNC_ADMIN_SECRET'] = 'My Admin Secret';
    const client = new Async();
    expect(client.apiKey).toBe('My API Key');
    expect(client.adminSecret).toBe('My Admin Secret');
  });

  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['ASYNC_API_KEY'] = 'another My API Key';
    process.env['ASYNC_ADMIN_SECRET'] = 'another My Admin Secret';
    const client = new Async({ apiKey: 'My API Key', adminSecret: 'My Admin Secret' });
    expect(client.apiKey).toBe('My API Key');
    expect(client.adminSecret).toBe('My Admin Secret');
  });
